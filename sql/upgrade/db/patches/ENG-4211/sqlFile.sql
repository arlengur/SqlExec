...
nptKits.each { pk ->
    ...
    def kitsWithMomSamples = cf.parentKits.findAll{  it.samples.any{  s->s.subject.familyPositions.any{  it == com.natera.lims.api.model.core.FamilyPosition.patient}}}
    def eddOrGa = ""
...
receivedKits.sort { a, b -> a.caseFile.id.compareTo(b.caseFile.id) }.eachWithIndex { pk, rowCount ->
  ...
  pk.receipts.find { k, v -> v.redrawReason?.name == "npt_sampleCollectionDateTooOld" } ?
      pk.samples.findAll { pk.caseFile.getFamilyPosition(it) == com.natera.lims.api.model.core.FamilyPosition.patient }.collect { "${it.barcode}: ${new SimpleDateFormat(''yyyy-MM-dd'').format(it.collectionDate)}" }.join(", ") : "",
  pk.receipts.find { k, v -> v.redrawReason?.name == "npt_shortDraw" } ?
      pk.samples.findAll { pk.caseFile.getFamilyPosition(it) == com.natera.lims.api.model.core.FamilyPosition.patient }.collect { "${it.barcode}: ${it.bloodVolume}" }.join(", ") : "",
...
if (setCaseState) {
  casefile.state = com.natera.lims.api.model.core.LimsState.valueOf(setCaseState)
...
def YES = QuestionnaireChoice.Y
def NO = QuestionnaireChoice.N
...
import com.natera.lims.api.model.core.TestType
import com.natera.lims.api.model.core.FamilyPosition
import com.natera.lims.api.model.core.LimsState
import com.natera.lims.datamodel.ivf.prod.SampleType
import org.apache.poi.xssf.usermodel.XSSFWorkbook
...
UPDATE `prodlims`.`script`
SET code = 'import com.natera.lims.api.model.core.FamilyPosition
import org.apache.poi.xssf.usermodel.XSSFWorkbook
import.com.natera.lims.api.model.core.*
import com.natera.lims.datamodel.ivf.prod.*;
...
subjectToAssign = cf.family.getMember(FamilyKey.getKey(com.natera.lims.api.model.core.FamilyPosition.patient), false)
